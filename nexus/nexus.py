...
DEFAULT_LOCAL = 'nexus.json'
OUTPUT_FILE = 'nexus.txt'


def load_nexus(source):
    """source: пустая строка → ./nexus.json, иначе путь к локальному файлу."""
...
def resolve_source():
    ...
    try:
        return input(
            "Путь до nexus.json (Enter — использовать ./nexus.json): "
        )
    except EOFError:
        return ''
...
def main():
    data = load_nexus(resolve_source())

    if 'components' not in data:
        raise RuntimeError("В файле отсутствует ключ 'components' на верхнем уровне.")

    output_lines = []
    for component_name, component_data in data['components'].items():
        version = component_data.get('version', '')
        git_data = component_data.get('git', {})
